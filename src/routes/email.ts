...
email.get('/verify', async (c) => {
  ...
  return c.html(
    page(
      'Email confirmed ✓',
      html`Alerts are now active for <strong>${sub.email}</strong>. We'll email you when a new malicious clone of your repositories appears.`,
      c.env.APP_URL,
    ),
  );
});
...
async function handleUnsubscribe(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  return c.html(
    page(
      'Unsubscribed',
      html`You won't receive any more impersonation alerts. You can re-enable them anytime from the app.`,
      c.env.APP_URL,
    ),
  );
}
...
// A branded type produced only by the `html` tagged template below.
// Callers cannot pass a plain string — the type system enforces that every
// body string was built through `html`, which escapes all interpolated values.
type SafeHtml = string & { readonly __safeHtml: unique symbol };

// Tagged template that auto-escapes every interpolated value. Static parts of
// the template string are kept verbatim, so intentional markup like <strong>
// can be authored directly; only runtime values are escaped.
...
// `title` and `appUrl` are escaped inside `page`. `body` must be produced by
// the `html` tagged template above, which escapes all interpolated values at
// construction time while allowing intentional static markup.
export function page(title: string, body: SafeHtml, appUrl: string): string {
