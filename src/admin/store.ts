...
import { buildDayBuckets } from '../scans/stats.js';
...
export async function getAdminStats(env: Env, now: number): Promise<AdminStats> {
  const since24h = now - DAY;
  const since7d = now - 7 * DAY;
  const since30d = now - 30 * DAY;
  const startOfTodayUtc = Date.parse(`${new Date(now).toISOString().slice(0, 10)}T00:00:00Z`);
  ...
  const [
    ...
    countWhere(env, `SELECT COUNT(*) AS n FROM users WHERE first_seen_at >= ?`, startOfTodayUtc),
    countAllScans(env),
